...
/**
 * Application for monitoring and configuring FIXED devices.
 */
@Component(immediate = true, service = FixedService.class)
public class FixedManager implements FixedService {

    private static final String APP_NAME = "org.onosproject.fixed";
    ...
    protected FixedStore fixedStore;
    ...
    public void setTargetPortPower(DeviceId deviceId, PortNumber portNumber, double power) {
        ...
        if (powerConfig != null) {
            fixedStore.setTargetPower(deviceId, portNumber, power);
    ...
    public Double getTargetPortPower(DeviceId deviceId, PortNumber portNumber) {
        ...
        Double power = fixedStore.getTargetPower(deviceId, portNumber);
        if (power == null) {
    ...
    public Double syncTargetPortPower(DeviceId deviceId, PortNumber portNumber) {
        ...
        if (powerConfig != null) {
            ...
            if (pl.isPresent()) {
                fixedStore.setTargetPower(deviceId, portNumber, pl.get());
                return pl.get();
            } else {
                fixedStore.removeTargetPower(deviceId, portNumber);
            }
        }
        return null;
    }
    ...
    private void initDevices() {
        for (Device device : deviceService.getDevices(Device.Type.ROADM)) {
            ...
            //FIXME
            // As fixed application is a optional tool for now.
    ...
    // Initialize FixedStore for a device to support target power
    private void initDevice(DeviceId deviceId) {
        if (!fixedStore.deviceAvailable(deviceId)) {
            fixedStore.addDevice(deviceId);
        }
    ...
    private void setAllInitialTargetPortPowers(DeviceId deviceId) {
        ...
        for (Port port : ports) {
            ...
            if (range.isPresent()) {
                Double power = fixedStore.getTargetPower(deviceId, port.number());
                if (power == null) {
                    // Set default to middle of the range
                    power = (range.get().lowerEndpoint() + range.get().upperEndpoint()) / 2;
                    fixedStore.setTargetPower(deviceId, port.number(), power);
                }
    ...
    // Tested with Lumentum FIXED-20 1 seconds was not enough, increased to 5 seconds
